...
function columns (table, schema) {
    if (x.dialect == 'mysql') {
        ...
        return "select c.name, type_name(c.system_type_id) as TypeName, c.max_length, c.precision, c.is_nullable, c.is_identity, " +
            "(select i.is_primary_key from sys.index_columns ik " +
            " inner join sys.indexes i on i.object_id = t.object_id and i.index_id = ik.index_id " +
            " where ik.column_id = c.column_id and ik.object_id = t.object_id and i.is_primary_key = 1) as isprimary" +
            " from sys.columns c" +
            " inner join sys.tables t on t.object_id = c.object_id" +
            " where t.name = " + x.wrap(table);
